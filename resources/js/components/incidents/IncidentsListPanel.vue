...
<template>
            ...
            >
                <div class="relative w-full sm:max-w-xs md:max-w-sm">
                    ...
                    <input
                        ...
                        class="flex h-9 w-full rounded-lg border border-border/60 bg-background/60 py-1 pl-9 pr-8 text-sm text-foreground shadow-sm backdrop-blur-sm placeholder:text-muted-foreground/50 transition-colors focus-visible:border-primary/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/60 hover:border-primary/25 hover:bg-background/80"
                    ...
                    <div class="flex min-w-0 flex-1 items-start gap-3 px-4 py-3.5">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <div class="mt-2 flex flex-wrap items-center gap-1.5">
                                ...
                                <Badge v-for="tag in ticket.tags" :key="tag" variant="secondary" class="border-border/50 px-1.5 py-0.5 text-[9px] font-medium">
                                    {{ tag }}
                                </Badge>
                            </div>
                        ...
                        <thead class="sticky top-0 z-10 border-b border-border/60 bg-muted/55 shadow-sm backdrop-blur-md dark:bg-muted/40">
                            <tr class="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                                <th v-if="!readonly" class="w-10 py-3.5 pl-5 pr-3">
                                    <Checkbox :checked="isAllSelected" aria-label="Select all incidents on this page" @update:checked="emit('toggleSelectAll')" />
                                </th>
                                ...
                                <th v-if="!readonly" class="w-24 px-4 py-3.5 pr-5 text-right">
                                    ...
                                    <span aria-hidden="true" class="text-[10px] font-normal normal-case tracking-normal text-muted-foreground/40">Actions</span>
                                </th>
                            </tr>
                        </thead>
                            ...
                            >
                                ...
                                <td class="px-3 py-3.5">
                                    <div class="flex max-w-sm flex-col">
                                        ...
                                        <div class="mt-1.5 flex flex-wrap items-center gap-1.5">
                                            ...
                                            <span class="text-[10px] font-medium uppercase tracking-tight text-muted-foreground/70">{{ ticket.category }}</span>
                                            <Badge
                                ...
                                <td class="px-4 py-3.5">
                                    <div v-if="ticket.handlers.length > 0" class="flex items-center gap-1.5">
                                        ...
                                        >
                                        <span v-if="ticket.handlers.length === 1" class="max-w-[90px] truncate text-xs font-medium text-muted-foreground">{{
                                            ticket.handlers[0].name
                                        }}</span>
                                    </div>
                                    <button
                                        v-else-if="!readonly"
                                        ...
                                        class="inline-flex items-center gap-1 rounded-md border border-dashed border-border/40 px-1.5 py-0.5 text-[10px] font-semibold italic text-muted-foreground/40 transition-colors hover:border-primary/30 hover:bg-primary/8 hover:text-primary"
                                ...
                                <td v-if="!readonly" class="px-4 py-3.5 pr-5 text-right" @click.stop>
                                    <div class="flex items-center justify-end gap-1">
                                        <button
                                            ...
                                            :disabled="statusProcessing === ticket.numericId"
                                            class="inline-flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground/0 transition-all duration-150 hover:bg-emerald-500/10 group-hover:text-emerald-500 disabled:opacity-50"
        ...
        >
            ...
            <div class="order-1 flex items-center gap-1 sm:order-2">
                ...
                <template v-for="(p, i) in pageRange" :key="i">
                    <span v-if="p === '...'" class="flex h-7 w-5 select-none items-center justify-center text-xs text-muted-foreground/50">…</span>
                    <button
                        v-else
                        type="button"
    ...
    <Teleport to="body">
        <Transition name="handler-tooltip">
            ...
            >
                <div class="rounded-lg border border-border bg-popover px-2.5 py-1.5 text-[11px] font-semibold whitespace-nowrap text-popover-foreground shadow-lg">
                    {{ handlerTooltip.name }}
                </div>
                <div class="mx-auto h-0 w-0 border-x-[5px] border-x-transparent border-t-[5px] border-t-border" />
            </div>
        </Transition>
    </Teleport>
